...
def SingleInvNet_combine(I1_ref,I2_ref,I3_ref, idi, L1):
    # Layer 1. order
    I_1_w11 = keras.layers.Dense(1, kernel_initializer=initializer_1, kernel_constraint=keras.constraints.NonNeg(),
                                 kernel_regularizer=keras.regularizers.l2(L1),
                                 use_bias=False, activation=None, name='w' + str(1 + idi) + '1')(I1_ref * I2_ref)
    I_1_w21 = keras.layers.Dense(1, kernel_initializer=initializer_exp, kernel_constraint=keras.constraints.NonNeg(),
                                 kernel_regularizer=keras.regularizers.l2(L1),
                                 use_bias=False, activation=activation_Exp, name='w' + str(2 + idi) + '1')(I1_ref * I2_ref)

    # Layer 2. order
    I_1_w31 = keras.layers.Dense(1, kernel_initializer=initializer_1, kernel_constraint=keras.constraints.NonNeg(),
                                 kernel_regularizer=keras.regularizers.l2(L1),
                                 use_bias=False, activation=None, name='w' + str(3 + idi) + '1')(I1_ref * I3_ref)
    I_1_w41 = keras.layers.Dense(1, kernel_initializer=initializer_exp, kernel_constraint=keras.constraints.NonNeg(),
                                 kernel_regularizer=keras.regularizers.l2(L1),
                                 use_bias=False, activation=activation_Exp, name='w' + str(4 + idi) + '1')(I1_ref * I3_ref)
...
def StrainEnergy_i5():
    ...
    I1_comnine_out = SingleInvNet_combine(I1_ref, I4_ref, I5_ref,4*terms,L1)
    I2_comnine_out = SingleInvNet_combine(I2_ref, I4_ref, I5_ref,5*terms,L1)
    
    ALL_I_out = [I1_out,I2_out,I4_out,I5_out,I1_comnine_out,I2_comnine_out]
    ALL_I_out = tf.keras.layers.concatenate(ALL_I_out,axis=1)
...
def Stress_xx_I5_UT(inputs):
    """
    计算横向各向同性材料在单轴拉伸下的 Cauchy 应力 σ_xx
    """
    (dPsidI1, dPsidI2, dWdI4, dWdI5, Stretch, I1, h11, h11_i5) = inputs

    # 计算横向拉伸比 λ_t
    Stretch_t = tf.math.pow(Stretch, -0.5)  # λ_t = 1/sqrt(λ_x)

    # 常量
    two = tf.constant(2.0, dtype=tf.float32)
    four = tf.constant(4.0, dtype=tf.float32)

    # 计算应力各项贡献
    stress_1 = two * (dPsidI1 + I1 * dPsidI2) * (Stretch**two - Stretch_t**two)
    stress_2 = two * dPsidI2 * (Stretch_t**four - Stretch**four)
    stress_3 = two * dWdI4 * h11
    stress_4 = four * dWdI5 * h11_i5
...
def modelArchitecture_I5(Psi_model,setAl, init):
    ...
    I2_BT = tf.keras.layers.Lambda(lambda x: 1/x[0]**2 + 2*x[0], name = 'I2')([Stretch_x, Stretch_y])
    ...
    # Stress XX
    Stress_xx_BT = keras.layers.Lambda(function = Stress_xx_I5_UT,
...
def modelArchitecture_I5_Uniaxial(Psi_model, setAl, init):
    # 轴向拉伸比 λ_x 作为输入
    Stretch_x = keras.layers.Input(shape=(1,), name='Stretch_x')

    # 体积守恒假设（不可压缩）：λ_y = λ_z = 1 / sqrt(λ_x)
    Stretch_y = tf.keras.layers.Lambda(lambda x: 1 / tf.sqrt(x), name='Stretch_y')(Stretch_x)

    # 计算应变不变量
    I1_UT = tf.keras.layers.Lambda(lambda x: x[0]**2 + 2 * x[1]**2, name='I1')([Stretch_x, Stretch_y])
    I2_UT = tf.keras.layers.Lambda(lambda x: (x[0]**2)*(x[1]**2) + 1/x[0]**2 + 1/x[1]**2, name='I2')([Stretch_x, Stretch_y])

    # 计算 H-Tensor 分量（基于皮肤的材料参数）
    Stretches_in = tf.keras.layers.concatenate([Stretch_x, Stretch_y], axis=1)
    h11, h22, h11_i5, h22_i5 = H_Layer_FungBiax_I4I5('alpha', setAl, init)(Stretches_in)

    # 计算 I4 和 I5（基于纤维方向的影响）
    I4_UT = tf.keras.layers.Lambda(lambda x: x[0] + x[1], name='I4')([h11, h22])
    I5_UT = tf.keras.layers.Lambda(lambda x: x[0] + x[1], name='I5')([h11_i5, h22_i5])

    # 计算应变能密度
    Psi_BT = Psi_model([I1_UT, I2_UT, I4_UT, I5_UT])

    # 计算应力导数（对 I1, I2, I4, I5）
    dWI1_UT = keras.layers.Lambda(lambda x: myGradient(x[0], x[1]))([Psi_BT, I1_UT])
    dWdI2_UT = keras.layers.Lambda(lambda x: myGradient(x[0], x[1]))([Psi_BT, I2_UT])
    dWdI4_UT = keras.layers.Lambda(lambda x: myGradient(x[0], x[1]))([Psi_BT, I4_UT])
    dWdI5_UT = keras.layers.Lambda(lambda x: myGradient(x[0], x[1]))([Psi_BT, I5_UT])

    # 计算应力（皮肤单轴拉伸应力）
    Stress_xx_UT = keras.layers.Lambda(function=Stress_xx_I5_UT,
                                       name='Stress_xx_BT')([dWI1_UT, dWdI2_UT, dWdI4_UT, dWdI5_UT,
                                                             Stretch_x, I1_UT, h11, h11_i5])

    # 定义单轴拉伸模型
    model_UT = keras.models.Model(inputs=[Stretch_x], outputs=[Stress_xx_UT])
